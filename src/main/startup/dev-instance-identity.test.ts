...
describe('dev-instance-identity', () => {
  ...
  it('pins a stable dev appName across branches so the safeStorage key does not churn', () => {
    ...
    expect(a.appName).toBe(`${BRAND.productName} Dev`)
    expect(b.appName).toBe(`${BRAND.productName} Dev`)
    expect(a.appName).not.toBe(BRAND.productName)
  })
  ...
  it('derives a readable dev label from worktree and branch env', () => {
    ...
    expect(identity.name).toBe(`${BRAND.productName}: nwparker/dev-indicator`)
    expect(identity.dockBadgeLabel).toBeNull()
    expect(identity.appUserModelId).toMatch(
      new RegExp(`^${BRAND.appId.replace(/\./g, '\\.')}\\.dev\\.[a-f0-9]{10}$`)
    )
  })
  ...
  it('includes the branch when it differs from the worktree basename', () => {
    ...
    expect(identity.name).toBe(`${BRAND.productName}: feature/billing-shell`)
    expect(identity.dockBadgeLabel).toBeNull()
  })
  ...
  it('allows an explicit label override', () => {
    ...
    expect(identity.name).toBe(`${BRAND.productName}: feature/other`)
    expect(identity.dockBadgeLabel).toBeNull()
  })
})
